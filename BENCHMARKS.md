...
TenzorBus does not win every single-consumer latency or throughput row. The
system is primarily designed around shared-memory fan-out and copy scaling:
one publication and one producer-side copy for an existing payload, with
zero-copy consumer views as consumer count grows. The complete matrix below is
reported without hiding the rows where another transport is faster.
...
The byte-identical raw JSON, original CSV, and human-readable report are under
`evidence/epyc-benchmark/`. See that directory's `README.md` for provenance and
integrity checks.
